...
@property (nonatomic, strong) UIButton *commitBugBtn;
...
-(instancetype)init{
    ...
    if (self) {
        ...
        UIView *panView = [[UIView alloc] initWithFrame:self.bounds];
        ...
        _viewBound.layer.borderWidth = 0.5;
        _viewBound.layer.borderColor = [UIColor doraemon_colorWithHex:0xCC3A4B].CGColor;
        _viewBound.layer.zPosition = FLT_MAX;
        _viewBound.userInteractionEnabled = NO;
        
        _viewInfoWindow = [[DoraemonVisualInfoWindow alloc] initWithFrame:CGRectMake(kDoraemonSizeFrom750(30), DoraemonScreenHeight - kDoraemonSizeFrom750(200) - kDoraemonSizeFrom750(30), DoraemonScreenWidth - 2*kDoraemonSizeFrom750(30), kDoraemonSizeFrom750(200))];
       [_viewInfoWindow makeKeyAndVisible];
        ...
        _commitBugBtn = [[UIButton alloc] initWithFrame:CGRectMake(_viewInfoWindow.bounds.size.width - closeWidth - kDoraemonSizeFrom750(32), CGRectGetMaxY(_closeBtn.frame) + 8, closeWidth, closeHeight)];
        [_commitBugBtn setTitle:@"bug" forState:UIControlStateNormal];
        [_commitBugBtn.titleLabel setFont:[UIFont systemFontOfSize:8]];
        [_commitBugBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [_commitBugBtn setBackgroundColor:[UIColor groupTableViewBackgroundColor]];
        [_commitBugBtn addTarget:self action:@selector(commitBug) forControlEvents:UIControlEventTouchUpInside];
        [_viewInfoWindow addSubview:_commitBugBtn];
        
        _viewInfoLabel = [[UILabel alloc] initWithFrame:CGRectMake(kDoraemonSizeFrom750(12), kDoraemonSizeFrom750(12), _viewInfoWindow.bounds.size.width - 2*kDoraemonSizeFrom750(12) - _closeBtn.doraemon_width , _viewInfoWindow.bounds.size.height - 2*kDoraemonSizeFrom750(12))];
...
- (void)commitBug {
    [[NSNotificationCenter defaultCenter] postNotificationName:BugCommitNotification object:nil];
